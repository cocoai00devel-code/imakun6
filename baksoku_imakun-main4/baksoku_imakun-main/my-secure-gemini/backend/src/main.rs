        ...
        tokio::select! {
            ...
            msg = browser_ws.next() => {
                if let Some(Ok(m)) = msg {
                    let g_msg = match m {
                        Message::Binary(b) => GMsg::Binary(b),
                        Message::Text(t) => GMsg::Text(t.into()),
                        _ => continue,
                    };
                    if gemini_ws.send(g_msg).await.is_err() { break; }
                } else { break; }
            }
            // 🤖 Geminiからの返答をブラウザへ
            msg = gemini_ws.next() => {
                if let Some(Ok(m)) = msg {
                    let b_msg = match m {
                        GMsg::Binary(b) => Message::Binary(b),
                        GMsg::Text(t) => Message::Text(t.into()),
                        _ => continue,
                    };
                    if browser_ws.send(b_msg).await.is_err() { break; }
                } else { break; }
            }
        }
    }
